...
function ToDoList({ listId, handleBackButton }){
    ...
    function handleCheckToggle(itemId, newState) {
        const updateData = async () => {
            const response = await axios.patch(`/api/lists/${listData.id}/checked_state`, {item_id: itemId, checked_state: newState});
            setListData (await response.data);
        };
        updateData();
    }
                    ...
                    listData.items.map((item) => {
                        return (
                            <div key={item.id} 
                                ...
                                <span>{item.checked ? "Done" : "ToDo"}</span>
                                <span className="label">{item.label}</span>
                                <span className="trash" 
                                          onClick={(evt) => {evt.stopPropagation(); handleDeleteItem(item.id);}}
                                >
                                    <BiSolidTrash/>
                                </span>
                                <span></span>
